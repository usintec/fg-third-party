...
onMounted(async () => {
  const response = await LookUpService.dailcodeFlag()
  ...
  if(props.item) {
    ...
    const index = Object.values(response).findIndex((v: any) => v.dialCode == passedFlag?.dialCode)
    if(index < 0)
      selectedItem.value = {
    ...
    else if(passedFlag) selectedItem.value = {...passedFlag, value: response.length > 0 ? response[index ?? 157] : undefined}
  } else
  selectedItem.value = {
    value: response.length > 0 ? response[157] : undefined,
    flag: 'https://api.normsmiway.com/static/country-flags/ng.svg',
